...
void EncodePagesResult(const std::vector<std::string>& iResult, v8::Handle<v8::Array> oResult) {
  ...
  for(std::vector<std::string>::const_iterator it = iResult.begin(); it != iResult.end(); ++it) {
    v8::Local<v8::Value> data = node::Buffer::New(MY_NODE_MODULE_ISOLATE, it->size()).ToLocalChecked();
    oResult->Set(i++, data);
  }
}
...
MY_NODE_MODULE_CALLBACK(render)
{
  ...
  v8::Local<v8::ArrayBuffer::Contents> dataobject = options->Get(V8_STRING_NEW_UTF8("data")).As<v8::ArrayBuffer::Contents>();
